...
def approach_ee_handle(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    ...
    # print("handle_pos1:", handle_pos.shape, handle_pos)
...
def align_ee_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    # print("drawer_handle_top:", env.scene["cabinet_frame"].data.target_pos_w[..., 0, :], handle_quat)  # env.scene["drawer_handle_top"].data.target_quat_w[:, :],
    ...
    align_x = torch.bmm(ee_tcp_y.unsqueeze(1), handle_y.unsqueeze(-1)).squeeze(-1).squeeze(-1)
    # print("align:", align_z, align_x, torch.sign(align_z) * align_z**2, torch.sign(align_x) * align_x**2)
    # return torch.sign(align_z) * align_z**2
    return (torch.sign(align_z) * align_z**2 + torch.sign(align_x) * align_x**2)
...
def align_grasp_around_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    # print("is_graspable:", rfinger_pos[:, 2], handle_pos[:, 2], lfinger_pos[:, 2], handle_pos[:, 2], is_graspable)
