...
class EarlyConvViTMultiViewImageCondition(BaseNNCondition):
    ...
    def __init__(
            ...
            in_channels: Tuple[int] = (3, ),
            qpos_vector_dim: Optional[int] = None,
            instruction_vector_dim: Optional[int] = None,
            To: int = 1,
        ...
        self.qpos_proj = nn.Linear(qpos_vector_dim, d_model) if qpos_vector_dim is not None else None
        self.qpos_emb = nn.Parameter(
            torch.zeros((1, 1, d_model)), requires_grad=True) if qpos_vector_dim is not None else None
        self.instruction_proj = nn.Linear(instruction_vector_dim, d_model) if instruction_vector_dim is not None else None
        self.instruction_emb = nn.Parameter(
            torch.zeros((1, 1, d_model)), requires_grad=True) if instruction_vector_dim is not None else None
    ...
    def image_token_lens(self):
        examples = [
            torch.randn((1, self.in_channels[i], self.image_sz[i][0], self.image_sz[i][1]))
            for i in range(self.n_views)]
    ...
    def forward(self, condition: Dict[str, torch.Tensor], mask: Optional[torch.Tensor] = None):
        ...
        b = condition["image"][0].shape[0]
        v = self.n_views
        tokens = []

        if self.qpos_proj is not None:
            tokens.append(
                self.qpos_proj(condition['qpos']) + self.qpos_emb)
        if self.instruction_proj is not None:
            tokens.append(
                self.instruction_proj(condition["instruction"]) + self.instruction_emb)
        ...
        if self.mask_cache is None or tokens.shape[1] != self.mask_cache.shape[1]:
            self.mask_cache = torch.tril(
                torch.ones(tokens.shape[1], tokens.shape[1], device=condition["image"][0].device), diagonal=0)
